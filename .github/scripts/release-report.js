...
module.exports = ({ context }) => {
    ...
    return `
Docker build preview for aristotle/apps is ready!
...
function getContainerPRLinks({ payload: { number } }) {
    ...
    return apps.map(app => `ghcr.io/imagyne/eatlater-${app}:pr-${number}`)
}

function getContainerHashLinks({ sha }) {
    const apps = process.env.APPS.split(',')
    return apps.map(app => `ghcr.io/imagyne/eatlater-${app}:${sha}`)
}
