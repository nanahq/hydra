...
export class SubscriptionService {
  ...
  async getUserSubscriptions (
    ...
    try {
      ...
      throw new FitRpcException(
        'Can not fetch subscription at this time',
  ...
  async createSubscriptionInstance (
    payload: SendApprovalPushNotification
  ): Promise<void> {
    try {
      ...
      if (payload.token !== undefined) {
        ...
        await this.pushNotificationClient.sendSingleNotification(
          payload.token,
          message
        )
      }
      ...
      throw new FitRpcException(
        'Can not create a subscription object for vendor',
        HttpStatus.INTERNAL_SERVER_ERROR
      )
    }
  }
  ...
  async subscribeUnsubscribeUser (payload: SubscribeDto): Promise<void> {
    try {
      ...
      const userIsSubscribed = sub.subscribers.find(
        (id) => id === payload.subscriberId
      )

      if (userIsSubscribed !== undefined) {
        newSubscribers = sub.subscribers.filter(
          (id) => id !== payload.subscriberId
        )
      } else {
      ...
      throw new FitRpcException(
        'Can not subscribe to notification at this moment',
        HttpStatus.INTERNAL_SERVER_ERROR
      )
    }
  }
  ...
  async updateSubscriptionSetting (
    payload: UpdateSubscriptionByVendorDto
  ): Promise<void> {
    try {
      await this.subscriptionRepository.findOneAndUpdate(
        { vendor: payload.vendor },
        { enabledByVendor: payload.enabledByVendor }
      )
    } catch (error) {
      throw new FitRpcException(
        'Can not update subscription settings at this time. Something went wrong',
        HttpStatus.INTERNAL_SERVER_ERROR
      )
    }
  }

  async sendPushMessageToSubscribers (
    payload: ScheduledPushPayload
  ): Promise<void> {
    try {
      const subscription: SubscriptionNotification =
        await this.subscriptionRepository.findOneAndPopulate(
          { vendor: payload.vendor },
          ['subscribers', 'vendor']
        )
      ...
      const availableDate = moment(payload.listingAvailableDate).format(
        'dddd Do'
      )
      const notificationTokens: any[] = subscription.subscribers
      ...
      await this.pushNotificationClient.sendMultipleNotifications(
        notificationTokens,
        message
      )
      ...
      throw new FitRpcException(
        'Can not send notification subscription to users at this time at this time. Something went wrong',
        HttpStatus.INTERNAL_SERVER_ERROR
      )
    }
  }
