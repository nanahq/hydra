...
export class NotificationServiceService {
 ...
 async  verifyPhone({code, phoneNumber}: PhoneVerificationPayload ) {
  try {
    const res =  await  this.twilioService.client.verify.v2
    .services(this.configService.get<string>('TWILIO_SERVICE_NAME'))
    .verificationChecks
    .create({to: phoneNumber, code,})

    if(res.status === 'approved') {
       await lastValueFrom(
           this.usersClient.send(QUEUE_MESSAGE.UPDATE_USER_STATUS, {phoneNumber})
        )
      return {status: 1}
    }
