...
export class VendorsController {
  ...
  async getHomepageData (
    ...
    try {
      return await this.vendorsService.getVendorsForHomepage(coordinates)
    } catch (error) {
      throw new RpcException(error)
    } finally {
      this.rmqService.ack(context)
  ...
  @EventPattern(QUEUE_MESSAGE.VENDORS_SEED_DATABASE)
  async seedDb (
    @Ctx() context: RmqContext
  ): Promise<void> {
    try {
  ...
  @MessagePattern(QUEUE_MESSAGE.VENDOR_SERVICE_REQUEST_PING)
  async ping (
    @Ctx() context: RmqContext
  ): Promise<string | undefined> {
    try {
      return await this.vendorsService.ping()
    } catch (error) {
      throw new RpcException(error)
    } finally {
      this.rmqService.ack(context)
