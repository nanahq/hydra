...
export class VendorController {
    ...
    async getAllVendors (
        ...
        return await lastValueFrom<VendorEntity[]>(
            this.vendorsClient.send(QUEUE_MESSAGE.GET_ALL_VENDORS, {})
                .pipe(
    ...
    async getVendor(
        ...
        return await lastValueFrom(
            this.vendorsClient.send(QUEUE_MESSAGE.GET_VENDOR, {vendorId})
                .pipe(
                    catchError((error: IRpcException) => {
                        throw new HttpException(error.message, error.status)
                    })
    ...
    async updateVendorStatus (
        ...
        return await lastValueFrom<{status: number}>(
            this.vendorsClient.send(QUEUE_MESSAGE.UPDATE_VENDOR_STATUS, data)
                .pipe(
                    catchError((error: IRpcException) => {
                        throw new HttpException(error.message, error.status)
                    })
    ...
    async deleteVendorProfile (
        ...
        return await lastValueFrom<{ status: number }>(
            this.vendorsClient.send(QUEUE_MESSAGE.DELETE_VENDOR_PROFILE, {userId}).pipe(
                catchError((error: IRpcException) => {
                    throw new HttpException(error.message, error.status)
                })
            )
