import { Body, Controller, Get, HttpException, Inject, Logger, Post, Put, UseGuards } from '@nestjs/common'
import { ClientProxy } from '@nestjs/microservices'

import {
  Admin,
  AdminLevel,
  CreateCouponDto, IRpcException,
  QUEUE_MESSAGE,
  QUEUE_SERVICE, ResponseWithStatus,
...
export class CouponController {
  ...
  public async createCoupon (
    ...
    return await lastValueFrom<ResponseWithStatusAndData<string>>(
      this.paymentClient.send(QUEUE_MESSAGE.CREATE_COUPON, data)
  ...
  public async updateCoupon (
    ...
    return await lastValueFrom<ResponseWithStatus>(
      this.paymentClient.send(QUEUE_MESSAGE.UPDATE_COUPON, data)
        .pipe(catchError((error: IRpcException) => {
          this.logger.error(JSON.stringify(error))
